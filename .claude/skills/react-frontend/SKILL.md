...
- The endpoint URL comes from build-time env config (e.g. `VITE_API_URL`), never hardcoded. Do not commit environment-specific values.
...
Run the project's typecheck, lint and test scripts (see `package.json`; update CLAUDE.md if commands are missing there).
